...
function resolveTargetFolderFromMatch(
  ...
  for (const seg of segments) {
    ...
    if (sawGlob && !isGlob) {
      return {
        ok: false,
        message: `Glob "${match}" matches multiple folders (literal segment "${seg}" appears after a glob). Split it into one rule per folder, e.g. folder_config({ action: "set-rule", rules: [{ match: "specs/foo/${seg}/**", ... }, ...] }).`,
      };
    }
    if (isGlob) {
      sawGlob = true;
