...
import { register } from './get-authoring-palette.ts';
...
function captureRegistration(): ToolHandler {
  ...
  const server = {
    ...
    tool() {
      throw new Error('legacy tool() should not be called by get_authoring_palette');
    },
...
describe('get_authoring_palette tool', () => {
