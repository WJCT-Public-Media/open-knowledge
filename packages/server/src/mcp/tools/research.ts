...
export const DESCRIPTION = [
  'Returns a step-by-step plan to research a topic: scan existing coverage, scope a rubric, capture sources via `ingest`, then write a provisional research article. Does NOT fetch or write — you execute the plan.',
  'Provisional, not canonical — findings live here until decisions solidify (`consolidate` promotes them).',
  'Use when exploring a decision space, comparing alternatives, or synthesizing sources into structured analysis.',
].join('\n');
