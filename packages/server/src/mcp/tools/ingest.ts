...
export const DESCRIPTION = [
  'Returns a step-by-step plan to capture an external source (URL or local file) into the knowledge base as raw, verbatim reference material — no analysis. Does NOT fetch or write — you execute the plan.',
  'Biases toward preserving raw binary files (PDF, image, audio, video) via the shell + `![[file.ext]]` embed, falling back to text extraction on shell-less hosts. The KB is closed-loop: cite the captured local doc, never a bare URL.',
  'Use when a source needs preserving before analysis — including URLs you fetched yourself to ground a KB claim.',
].join('\n');
