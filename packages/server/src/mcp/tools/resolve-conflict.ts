...
export const DESCRIPTION = [
  ...
  '',
  'Strategy:',
  '- `mine` — writes `git show :2:<file>` (your committed ours stage). Fails on delete-modify (DU) conflicts where stage 2 is missing — use `delete` instead.',
  '- `theirs` — writes `git show :3:<file>` (their committed stage). Fails on modify-delete (UD) conflicts where stage 3 is missing — use `delete` instead.',
  ...
  '- `delete` — runs `git rm <file>` then commits the deletion. Honors deletion intent for delete-modify (DU: "keep deletion") and modify-delete (UD: "accept their deletion") shapes. Inspect the `kind` field on `get_conflict_content` to pick the right strategy for the conflict shape.',
  '',
  'Returns 200 on success; 500 indicates commit failure (re-call `list_conflicts` to confirm post-state — the resolve API is best-effort, non-atomic, and the file may have been resolved by another session).',
  '',
