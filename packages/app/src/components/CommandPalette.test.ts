...
describe('Switch Project entry (source-level guards)', () => {
  ...
  test('Switch Project block contains LayoutGrid icon, "Switch Project" label, shortcut hint, and bridge.navigator.open() in the SAME CommandItem', () => {
    expect(
      switchProjectBlock,
      'CommandItem with command-palette-switch-project not found',
    ).toBeTruthy();
    expect(switchProjectBlock).toContain('<LayoutGrid');
    expect(switchProjectBlock).toContain('<Trans>Switch Project</Trans>');
    expect(switchProjectBlock).toContain("{formatShortcut('switch-project')}");
...
describe('Settings entry (US-010 / FR-1 / D54 — source-level guards)', () => {
  ...
  test('Settings CommandItem carries the icon, shortcut hint, and the Settings label', () => {
    ...
    expect(settingsBlock).toContain('<Trans>Settings</Trans>');
    expect(settingsBlock).toContain("{formatShortcut('settings')}");
  });
  ...
  test('Settings sits inside the "Project" CommandGroup', () => {
    ...
    const agentGroupIdx = src.indexOf('heading={t`Open with AI`}');
...
describe('Initialize starter pack entry (source-level guards)', () => {
  ...
  test('CommandItem carries the Package icon and "Initialize starter pack" label', () => {
    ...
    expect(seedBlock).toContain('<Trans>Initialize starter pack</Trans>');
  });
